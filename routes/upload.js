...
router.post('/excel_users', uploadExcel.single('file'), async function (req, res, next) {
    ...
    if (!userRole) {
        return res.status(400).send({ message: "Role 'user' không tồn tại, vui lòng tạo role trước" })
    }
    ...
    for (let row = 2; row <= worksheet.rowCount; row++) {
        ...
        let email = contentRow.getCell(2).value;

        if (!username || !email) {
            result.push({ row: row, error: "username hoặc email trống" })
            continue;
        }

        // Gen password ngẫu nhiên 16 ký tự
        let password = crypto.randomBytes(12).toString('base64url').slice(0, 16)
        ...
        try {
            ...
            try {
                ...
                console.log(`Không gửi được mail cho ${email}:`, mailErr.message)
            }

            result.push({ row: row, username: username, email: email, status: "success" })
        } catch (error) {
            result.push({ row: row, username: username, error: error.message })
        }
    }
    res.send(result)
})
